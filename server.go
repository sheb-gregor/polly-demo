...
func GetServer(broker *server.Broker) http.Handler {
	mux := chi.NewMux()
	...
	mux.Get("/poll", func(w http.ResponseWriter, r *http.Request) {
		...
		msg := broker.Poll(req.Topic, req.Subscriber)
		if msg == nil {
			writeData(w, http.StatusNotFound, Ok{Message: http.StatusText(http.StatusNotFound)})
			return
		}
	...
	mux.Post("/publish", func(w http.ResponseWriter, r *http.Request) {
		...
		writeSuccess(w, Ok{Message: http.StatusText(http.StatusOK)})
	})
...
func writeError(w http.ResponseWriter, err error) {
	...
	writeData(w, http.StatusBadRequest, Ok{Message: err.Error()})
}
